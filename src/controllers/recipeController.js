...
module.exports = {
  ...
  deleteRecipe: (req, res) => {
    ...
    Promise.all([
      ...
      .then((result) => {
        const delImg = result[0].data[0]
        const delVideo = result[1].data
        if (result[2]) {
        ...
        }   //berhasil delete pada tabel, Fs belum di cek
        res.status(200).json({
  ...
  addVideo: (req, res) => {
    const { recipeId } = req.params
    let bodyVideo;
    if (!req.files.videos) { //cannot read property of undefined
